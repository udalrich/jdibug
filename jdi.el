  ...
  (lexical-let ((jdi jdi)
    ...
    (condition-case err
		(ado ()
		  ...
		  (jdi-get-idsizes jdi)
		  (jdwp-send-command (jdi-jdwp jdi) "set" `((:event-kind . ,jdwp-event-class-prepare) 
													(:suspend-policy . 0) 
													(:modifiers . 0)))
		  (progn
			(jdwp-send-command (jdi-jdwp jdi) "set" `((:event-kind . ,jdwp-event-class-unload) 
													  (:suspend-policy . 0) 
													  (:modifiers . 0)))
			(jdwp-send-command (jdi-jdwp jdi) "set" `((:event-kind . ,jdwp-event-exception) 
													  ...
													  (:modifier
													   ((:mod-kind . 8)
														...
														(:caught . 0)
														(:uncaught . 1)))))
			(jdwp-send-command (jdi-jdwp jdi) "set" `((:event-kind . ,jdwp-event-thread-end) 
													  (:suspend-policy . 0) 
													  (:modifiers . 0)))
			(jdwp-send-command (jdi-jdwp jdi) "set" `((:event-kind . ,jdwp-event-thread-start)
													  (:suspend-policy . 0) 
													  (:modifiers . 0)))
			(jdwp-send-command (jdi-jdwp jdi) "set" `((:event-kind . ,jdwp-event-vm-death)
													  (:suspend-policy . 0) 
													  (:modifiers . 0)))
			(jdi-get-version jdi)
...
(defun jdi-methods-find-by-id (class method-id)
  ...
  (let ((result (find-if (lambda (method)
    ...
    (if (null result)
		(jdi-error "failed to find method with id:%s" method-id))
    result))
...
(defun jdi-values-slot-resolve (jdi thread frame values)
  ...
  (let ((data `((:thread . ,thread)
    ...
    (ado (jdi values) 
      ...
      (destructuring-bind (reply error id jdwp) ado-last-return-value
		(unless (= error jdwp-error-invalid-frameid)
		  (let ((i 0))
...
(defun jdi-values-resolve (jdi values)
  ...
  (dolist (value values)
    ...
    (cond ((or (= (jdi-value-type value) jdwp-tag-int)
			   (= (jdi-value-type value) jdwp-tag-byte)
			   (= (jdi-value-type value) jdwp-tag-char)
			   (= (jdi-value-type value) jdwp-tag-short))
		   (setf (jdi-value-string value) (format "%d" (jdi-value-value value))))
		  ((= (jdi-value-type value) jdwp-tag-long)
		   (setf (jdi-value-string value) (format "%d" (jdwp-vec-to-int (jdi-value-value value)))))
		  ((= (jdi-value-type value) jdwp-tag-float)
		  ...
		  ((= (jdi-value-type value) jdwp-tag-string)
		   (jdi-value-resolve-string jdi value))
		  ((= (jdi-value-type value) jdwp-tag-boolean)
		   (setf (jdi-value-string value) (if (= 0 (jdi-value-value value)) "false" "true")))
		  (t 
		   (jdi-error "fixme: do not know how to print value of type:%d name:%s" (jdi-value-type value) (jdi-value-name value))
